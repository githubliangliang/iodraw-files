...
sequenceDiagram
    ...
    前端->>Amazon页面: 跳转至授权URL并登录
    Amazon页面-->>后端: 3.返回授权code
    %% 步骤4：后端通过code获取token并写入Redis
    后端->>Amazon页面: 4.用code请求获取token
    disa-->>后端: 返回token
    后端->>Redis: 写入token
    ...
    loop 每15分钟执行（检测/刷新token）
        ...
        alt token即将过期/已过期
            后端->>Amazon页面: 请求刷新token
            Amazon页面-->>后端: 返回新token
            后端->>Redis: 更新为新token
        else token有效
            后端->>后端: 无需刷新，任务结束
        end
